...
def get_aspect_from_two_points(aspects_settings: dict, point_one: Union[float, int], point_two: Union[float, int], p1_num_sign: int, p2_num_sign: int):
    """
    Utility function.
    ...
    if int(distance) < aspects_settings[0]["orb"] and abs(p1_num_sign - p2_num_sign) == 0:
        ...
        <= int(distance)
        < (aspects_settings[3]["degree"] + aspects_settings[3]["orb"]) and 
        (abs(p1_num_sign - p2_num_sign) == 2 or abs((p1_num_sign - p2_num_sign)) == 10)
    ):
        ...
        <= int(distance)
        < (aspects_settings[5]["degree"] + aspects_settings[5]["orb"])) and 
        (abs(p1_num_sign - p2_num_sign) == 3 or abs((p1_num_sign - p2_num_sign )) == 9)
    ):
        ...
        <= int(distance)
        < (aspects_settings[6]["degree"] + aspects_settings[6]["orb"]) and 
        (abs(p1_num_sign - p2_num_sign) == 4 or abs((p1_num_sign - p2_num_sign)) == 8)
    ):
        ...
        <= int(distance)
        < (aspects_settings[10]["degree"] + aspects_settings[10]["orb"])) and 
        abs(p1_num_sign - p2_num_sign) == 6 
    ):
...
def get_active_points_list_ruler(subject: AstrologicalSubject, settings: Union[KerykeionSettingsModel, dict]) -> list:
    point_list = []
    for planet in settings["celestial_points"]:
        if planet["is_active"] == True:
            point_list.append(planet["related_zodiac_signs"])
